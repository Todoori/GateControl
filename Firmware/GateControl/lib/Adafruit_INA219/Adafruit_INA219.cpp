...
void Adafruit_INA219::init() {
  ...
  setINA219Configuration_18V_2A_033Ohm() ;
}
...
void Adafruit_INA219::setINA219Configuration_18V_2A_033Ohm() {
//By default we use a pretty huge range for the input voltage,
  // which probably isn't the most appropriate choice for system
  // that don't use a lot of power.  But all of the calculations
  // are shown below if you want to change the settings.  You will
  ...
  // VBUS_MAX = 32V		(Assumes 32V, can also be set to 16V)
  // VSHUNT_MAX = 0.08 (closest to 0.066)	(Assumes Gain 8, 320mV, can also be 0.16, 0.08, 0.04)
  // RSHUNT = 0.033	(Resistor value in ohms)
  ...
  // 3. Calculate possible range of LSBs (Min = 15-bit, Max = 12-bit)
  // MinimumLSB = MaxExpected_I/32767
  // MinimumLSB = 0.0000610             (61.1uA per bit)
  // MaximumLSB = MaxExpected_I/4096
  // MaximumLSB = 0.000488              (488uA per bit)

  // 4. Choose an LSB between the min and max values
  //    (Preferrably a roundish number close to MinLSB)
  // CurrentLSB = 0.0000700 (70uA per bit)

  // 5. Compute the calibration register
  // Cal = trunc (0.04096 / (Current_LSB * RSHUNT))
  // Cal = 17731

  ina219_calValue = 17731;

  // 6. Calculate the power LSB
  // PowerLSB = 20 * CurrentLSB
  // PowerLSB = 0.0014 (1,4mW per bit)

  // 7. Compute the maximum current and shunt voltage values before overflow
  //
  // Max_Current = Current_LSB * 32767
  // Max_Current = 2.293A before overflow
  //
  // If Max_Current > Max_Possible_I then
  //    Max_Current_Before_Overflow = MaxPossible_I
  // Else
  ...
  // 8. Compute the Maximum Power
  // MaximumPower = Max_Current_Before_Overflow * VBUS_MAX
  // MaximumPower = 2.293 * 32V
  // MaximumPower = 73.376W

  // Set multipliers to convert raw current/power values
  ina219_currentDivider_mA = 14.2857;    // Current LSB = 40uA per bit (1000/40 = 25)
  ina219_powerMultiplier_mW = 1.4f; // Power LSB = 800uW per bit
